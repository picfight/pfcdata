...
type RateClient interface {
	SendExchangeUpdate(*pfcrates.ExchangeRateUpdate) error
	Stream() GRPCStream
}
...
type GRPCStream interface {
	Send(*pfcrates.ExchangeRateUpdate) error
...
// SubscribeExchanges is a gRPC method defined in pfcrates/pfcrates.proto. It
// satisfies the PFCRatesServer interface. The gRPC server will call this method
// when a client subscription is received.
func (server *RateServer) SubscribeExchanges(hello *pfcrates.ExchangeSubscription, stream pfcrates.PFCRates_SubscribeExchangesServer) (err error) {
...
func (server *RateServer) ReallySubscribeExchanges(hello *pfcrates.ExchangeSubscription, stream GRPCStream) (err error) {
	...
	// Send PicFight exchanges.
	err = sendStateList(client, state.PfcBtc)
	if err != nil {
		return err
	}
	// Send Bitcoin-fiat indices.
	for token := range state.FiatIndices {
		err = client.SendExchangeUpdate(&pfcrates.ExchangeRateUpdate{
			Token:   token,
...
func (server *RateServer) addClient(stream GRPCStream, hello *pfcrates.ExchangeSubscription) (RateClient, StreamID) {
...
func makeExchangeUpdate(token string, state *exchanges.ExchangeState) *pfcrates.ExchangeRateUpdate {
	return &pfcrates.ExchangeRateUpdate{
		Token:      token,
		Price:      state.Price,
...
func (client *rateClient) SendExchangeUpdate(update *pfcrates.ExchangeRateUpdate) (err error) {

...
// AgendaTagged has the same fields as pfcjson.Agenda plus the VoteVersion
...
type DeploymentSource interface {
	GetVoteInfo(version uint32) (*pfcjson.GetVoteInfoResult, error)
}
...
// pfcjson.GetVoteInfoResult and chaincfg.ConsensusDeployment hold almost similar
// data contents but chaincfg.Vote does not contain the important vote status
// field that is found in pfcjson.Agenda.

...
export default class extends Controller {
  ...
  async connect () {
    ...
    // Parse stimulus data
    var cdata = ctrl.data
    ctrl.pfcAddress = cdata.get('pfcaddress')
  ...
  async showQRCode () {
    ...
    if (this.qrCode) {
      ...
      let qrCodeImg = await QRCode.toDataURL(this.pfcAddress,
        {
  ...
  makeTableUrl (txType, count, offset) {
    return '/addresstable/' + this.pfcAddress + '?txntype=' +
  ...
  async fetchGraphData (chart, bin) {
    ...
    let url = '/api/address/' + ctrl.pfcAddress + '/' + chartKey + '/' + bin

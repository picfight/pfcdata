...
func (*testClient) GetVoteInfo(version uint32) (*pfcjson.GetVoteInfoResult, error) {
	if version != 5 {
		return &pfcjson.GetVoteInfoResult{}, fmt.Errorf(voteVersionErrMsg, version)
	}
	resp := &pfcjson.GetVoteInfoResult{
		...
		Agendas: []pfcjson.Agenda{
			{
				ID:             "TestAgenda0001",
				Description:    "This agenda just shows pfcjson.GetVoteInfoResult payload format",
				Mask:           6,

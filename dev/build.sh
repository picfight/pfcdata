#!/bin/bash
# Usage: build.sh [pfcdata_root] [destination_folder]
#
...
#   root folder. If not running from a git repository, the pfcdata_root folder
#   must be specified.
#
#   Specify destination_folder to install the pfcdata executable and the static
...
if [[ -z "$ROOT" ]]; then
    echo "Not in git repository. You must specify the pfcdata root folder as the first argument!"
    exit 1
fi
...
echo "Building the pfcdata binary..."
GO111MODULE=on go build -v -o pfcdata
...
if [[ -n "$DEST" ]]; then
    sudo install -m 754 -o pfcdata -g picfight ./pfcdata ${DEST}/

...
RateServer is a central server for distributing PicFight exchange rate data to
any number of clients. It is particularly useful for PFCData instances behind a
...
```
cd path/to/pfcdata/pfcrates/rateserver
go build
./rateserver
```

See [PFCData's notes on Go](https://github.com/picfight/pfcdata#building-pfcdata-with-go-111)
...
### Using RateServer with PFCData

To point PFCData to RateServer, you must set PFCData's `ratemaster` and
...
-c, --config=            Path to a custom configuration file.
    --appdir=            Path to application home directory. (~/.pfcrates)

...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS bool) (*rpcclient.Client, semver.Semver, error) {
	...
	pfcdClient, err := rpcclient.New(connCfgDaemon, nil)
	if err != nil {
...
func TestIsZeroHashP2PHKAddress(t *testing.T) {
	...
	testIsZeroHashP2PHKAddress(mainnetDummy, &chaincfg.DecredNetParams, positiveTest)
	...
	// wrong network
	testIsZeroHashP2PHKAddress(testnetDummy, &chaincfg.DecredNetParams, negativeTest)
	...
	// wrong address
	testIsZeroHashP2PHKAddress("", &chaincfg.DecredNetParams, negativeTest)

...
type BlockFetcher interface {
	...
	GetBlockHeaderVerbose(hash *chainhash.Hash) (*pfcjson.GetBlockHeaderVerboseResult, error)
}
...
type blockData struct {
	...
	headerResult *pfcjson.GetBlockHeaderVerboseResult
}
...
func (p *BlockPrefetchClient) GetBlockData(hash *chainhash.Hash) (*wire.MsgBlock, *pfcjson.GetBlockHeaderVerboseResult, error) {
	p.Lock()
...
// GetBlockHeaderVerbose retrieves the pfcjson.GetBlockHeaderVerboseResult for
// the block with the specified hash. See GetBlockData for details on how this
// interacts with the prefetcher
func (p *BlockPrefetchClient) GetBlockHeaderVerbose(hash *chainhash.Hash) (*pfcjson.GetBlockHeaderVerboseResult, error) {
...
// becomes "current".
func (p *BlockPrefetchClient) storeNext(msgBlock *wire.MsgBlock, headerResult *pfcjson.GetBlockHeaderVerboseResult) {
	p.current = p.next
	p.next = &blockData{
...
func (p *BlockPrefetchClient) retrieveBlockAndHeaderResult(hash *chainhash.Hash) (*wire.MsgBlock, *pfcjson.GetBlockHeaderVerboseResult, uint32, error) {

...
func openDB() (func() error, error) {
	dbi := pfcpg.DBInfo{
		Host:   "localhost",
		Port:   "5432",
		User:   "pfcdata", // postgres for admin operations
		Pass:   "",
		DBName: "pfcdata_mainnet_test",
	}
	var err error
	// Connect to the PostgreSQL daemon and return the *sql.DB.
	db, err = pfcpg.Connect(dbi.Host, dbi.Port, dbi.User, dbi.Pass, dbi.DBName)
	if err != nil {
		return nil, err
	}
	if err = pfcpg.DropTestingTable(db); err != nil {
		return nil, err
	}
	if err = pfcpg.CreateTable(db, "testing"); err != nil {
		return nil, err
	}
...
func TestTimeRoundTripCorrectTimeDef(t *testing.T) {
	...
	if err := pfcpg.ClearTestingTable(db); err != nil {

...
func main() {
	...
	pfcrates.RegisterPFCRatesServer(grpcServer, rateServer)
		...
		for {
			...
			case update := <-xcSignals.Update:
				...
				if exchanges.IsPfcExchange(update.Token) {
					...
					grpcUpdate = &pfcrates.ExchangeRateUpdate{
						Token:   update.Token,

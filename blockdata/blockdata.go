...
// it into the BlockDataBasic and StakeInfoExtended and pfcdataapi types.
...
func (t *Collector) CollectBlockInfo(hash *chainhash.Hash) (*apitypes.BlockDataBasic,
	...
	*apitypes.BlockExplorerExtraInfo, *wire.MsgBlock, error) {
	...
	nbSubsidy, err := t.pfcdChainSvr.GetBlockSubsidy(int64(msgBlock.Header.Height)+1, 5)
	if err != nil {
...
func (t *Collector) Collect() (*BlockData, *wire.MsgBlock, error) {
	...
	// Stake difficulty
	stakeDiff, err := t.pfcdChainSvr.GetStakeDifficulty()
	if err != nil {
		return nil, nil, err
	}

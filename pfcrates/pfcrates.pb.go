...
func (c *dCRRatesClient) SubscribeExchanges(ctx context.Context, in *ExchangeSubscription, opts ...grpc.CallOption) (PFCRates_SubscribeExchangesClient, error) {
	stream, err := c.cc.NewStream(ctx, &_PFCRates_serviceDesc.Streams[0], "/dcrrates.PFCRates/SubscribeExchanges", opts...)
	if err != nil {
		return nil, err
	}
...
func _PFCRates_SubscribeExchanges_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(PFCRatesServer).SubscribeExchanges(m, &dCRRatesSubscribeExchangesServer{stream})
}

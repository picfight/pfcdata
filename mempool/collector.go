...
func (t *MempoolDataCollector) mempoolTxns() ([]exptypes.MempoolTx, txhelpers.MempoolAddressStore, txhelpers.TxnsStore, error) {
	mempooltxs, err := t.pfcdChainSvr.GetRawMempoolVerbose(pfcjson.GRMAll)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("GetRawMempoolVerbose failed: %v", err)
	}

	blockHash, _, err := t.pfcdChainSvr.GetBestBlock()
	if err != nil {
	...
	for hashStr, tx := range mempooltxs {
		...
		rawtx, err := rpcutils.GetTransactionVerboseByID(t.pfcdChainSvr, hash)
		if err != nil {
			log.Warn(err)
			continue
		}
		...
		txhelpers.TxPrevOutsByAddr(addrMap, txnsStore, msgTx, t.pfcdChainSvr, t.activeChain)
...
func ParseTxns(txs []exptypes.MempoolTx, params *chaincfg.Params, lastBlock *BlockID) *exptypes.MempoolInfo {
	...
	var totalOut, regularTotal, ticketTotal, voteTotal, revTotal pfcutil.Amount
	var likelyMineable bool
	var likelyTotal pfcutil.Amount
	...
	for _, tx := range txs {
		likelyMineable = true
		out, _ := pfcutil.NewAmount(tx.TotalOut) // 0 for invalid amounts
		switch tx.Type {
		case "Ticket":

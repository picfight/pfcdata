...
- db/pfcpg: Charts data updates could use incremental changes. (Issue)
...
```sh
git remote add upstream https://github.com/picfight/pfcdata.git
git fetch upstream
...
[Code of Conduct](https://github.com/picfight/pfcdata/blob/master/CODE_OF_CONDUCT.md).
...
`dcdrrates/runprotoc.sh` to recompile `pfcrates.pb.go`.

...
func (exp *explorerUI) Home(w http.ResponseWriter, r *http.Request) {
	...
	if xcBot != nil {
		conversions = &homeConversions{
			...
			CoinSupply:      xcBot.Conversion(pfcutil.Amount(homeInfo.CoinSupply).ToCoin()),
			PowSplit:        xcBot.Conversion(pfcutil.Amount(homeInfo.NBlockSubsidy.PoW).ToCoin()),
			TreasurySplit:   xcBot.Conversion(pfcutil.Amount(homeInfo.NBlockSubsidy.Dev).ToCoin()),
			TreasuryBalance: xcBot.Conversion(pfcutil.Amount(homeInfo.DevFund).ToCoin()),
		}
	}
...
func (exp *explorerUI) TxPage(w http.ResponseWriter, r *http.Request) {
	...
	// If pfcd has no information about the transaction, pull the transaction
	...
	if exp.liteMode {
		...
		if tx.Type == "Coinbase" {
			...
			if data == nil {
				...
				feeAmt, _ := pfcutil.NewAmount(data.MiningFee)
...
func (exp *explorerUI) AddressPage(w http.ResponseWriter, r *http.Request) {
	...
	if exp.xcBot != nil && !exp.liteMode {
		conversion = exp.xcBot.Conversion(pfcutil.Amount(addrData.Balance.TotalUnspent).ToCoin())
	}
...
func (exp *explorerUI) Search(w http.ResponseWriter, r *http.Request) {
	...
	// If block search failed, and pfcdata is in full mode, check the aux DB,
...
func (exp *explorerUI) commonData(r *http.Request) *CommonPageData {
	...
	darkMode, err := r.Cookie("pfcdataDarkBG")
	if err != nil && err != http.ErrNoCookie {
		log.Errorf("Cookie pfcdataDarkBG retrieval error: %v", err)
	}

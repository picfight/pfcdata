...
func (mpi *MempoolInfo) Trim() *TrimmedMempoolInfo {
	...
	getTotalFee := func(txs []*TrimmedTxInfo) pfcutil.Amount {
		var sum pfcutil.Amount
		for _, tx := range txs {
			sum += tx.TxBasic.Fee
		}
		return sum
...
func TrimMempoolTx(txs []MempoolTx) (trimmedTxs []*TrimmedTxInfo) {
	for _, tx := range txs {
		fee, _ := pfcutil.NewAmount(tx.Fees) // non-nil error returns 0 fee
		var feeRate pfcutil.Amount
		if tx.Size > 0 {
			feeRate = fee / pfcutil.Amount(int64(tx.Size))
		}
		txBasic := &TxBasic{
			TxID:          tx.TxID,

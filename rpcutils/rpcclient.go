...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS, disableReconnect bool,
	...
	// Check if the pfcd RPC API version is compatible with pfcdata.
...
func GetBlockHeaderVerbose(client BlockFetcher, idx int64) *pfcjson.GetBlockHeaderVerboseResult {
...
func GetBlockHeaderVerboseByString(client BlockFetcher, hash string) *pfcjson.GetBlockHeaderVerboseResult {
...
func GetBlock(ind int64, client BlockFetcher) (*pfcutil.Block, *chainhash.Hash, error) {
...
func GetBlockByHash(blockhash *chainhash.Hash, client BlockFetcher) (*pfcutil.Block, error) {
	msgBlock, err := client.GetBlock(blockhash)
	if err != nil {

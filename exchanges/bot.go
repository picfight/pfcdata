...
const (
	...
	defaultPFCRatesPort = "7778"
)

var grpcClient pfcrates.PFCRatesClient
...
type ExchangeBot struct {
	mtx             sync.RWMutex
	PfcBtcExchanges map[string]Exchange
	IndexExchanges  map[string]Exchange
	Exchanges       map[string]Exchange
	// BtcIndex is the (typically fiat) currency to which the PFC price should be
	...
	// exchanges or no up-to-date PicFight exchanges. IsFailed is a getter for failed.
	failed bool
}

// ExchangeBotState is the current known state of all exchanges, in a certain
// base currency, and a volume-averaged price and total volume in PFC.
type ExchangeBotState struct {
	BtcIndex    string                    `json:"btc_index"`
	Price       float64                   `json:"price"`
	Volume      float64                   `json:"volume"`
	PfcBtc      map[string]*ExchangeState `json:"dcr_btc_exchanges"`
...
func (state ExchangeBotState) copy() *ExchangeBotState {
	state.PfcBtc = copyStates(state.PfcBtc)
...
func (signal UpdateSignal) TriggerState() (xcState *ExchangeState, err error) {
	if IsPfcExchange(signal.Token) {
		xcState = signal.State.PfcBtc[signal.Token]
...
func NewExchangeBot(config *ExchangeBotConfig) (*ExchangeBot, error) {
	...
	bot := &ExchangeBot{
		PfcBtcExchanges: make(map[string]Exchange),
		...
		currentState: ExchangeBotState{
			...
			Price:       0,
			Volume:      0,
			PfcBtc:      make(map[string]*ExchangeState),
	...
	for token, constructor := range PfcExchanges {
		buildExchange(token, constructor, bot.PfcBtcExchanges)
	}

	if len(bot.PfcBtcExchanges) == 0 {
		return nil, fmt.Errorf("No PFC-BTC exchanges were intitialized")
	}
...
func (bot *ExchangeBot) Start(ctx context.Context, wg *sync.WaitGroup) {
	...
	if config.MasterBot != "" {
		...
		if err != nil {
			...
			// Start a loop to listen for updates from the pfcrates server.
			go func() {
				for {
					...
					if err != nil {
						...
						log.Errorf("PFCRates error. Attempting to reconnect in 1 minute: %v", err)
						...
						for {
							...
							if err == nil {
								...
								log.Errorf("Failed to reconnect to PFC Rates. Attempting to reconnect in 1 minute: %v", err)
							}
						}
						continue
					}
...
// Attempt PFCRates connection after delay.
func (bot *ExchangeBot) connectMasterBot(ctx context.Context, delay time.Duration) (pfcrates.PFCRates_SubscribeExchangesClient, error) {
	if bot.masterConnection != nil {
		bot.masterConnection.Close()
	}
	if delay > 0 {
	...
	grpcClient := pfcrates.NewPFCRatesClient(conn)
	stream, err := grpcClient.SubscribeExchanges(ctx, &pfcrates.ExchangeSubscription{
...
func (bot *ExchangeBot) ConvertedState(code string) (*ExchangeBotState, error) {
	...
	pfcPrice, volume := bot.processState(bot.currentState.PfcBtc, true)
	btcPrice, _ := bot.processState(fiatIndices, false)
	if pfcPrice == 0 || btcPrice == 0 {
		bot.failed = true
	...
	state := ExchangeBotState{
		...
		Price:       pfcPrice * btcPrice,
		PfcBtc:      bot.currentState.PfcBtc,
...
// updateExchange processes an update from a PicFight-BTC Exchange.
func (bot *ExchangeBot) updateExchange(update *ExchangeUpdate) error {
	bot.mtx.Lock()
	defer bot.mtx.Unlock()
	bot.currentState.PfcBtc[update.Token] = update.State
...
func (bot *ExchangeBot) updateState() error {
	...
	if pfcPrice == 0 || btcPrice == 0 {
		...
		bot.failed = false
		bot.currentState.Price = pfcPrice * btcPrice
...
// sources or no PicFight Exchanges are up-to-date. Individual exchanges can
// be outdated/failed without IsFailed being false, as long as there is at least
// one Bitcoin index and one PicFight exchange.
...
// Conversion is a representation of some amount of PFC in another index.

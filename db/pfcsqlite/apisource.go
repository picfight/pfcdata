...
func (db *WiredDB) ReportHeights() error {
	...
	if stakeInfoHeight != summaryHeight {
		err = fmt.Errorf("SQLite database (pfcdata.sqlt.db) is corrupted")
	}
...
func (db *WiredDB) BlockchainInfo() (*pfcjson.GetBlockChainInfoResult, error) {
...
func (db *WiredDB) GetAllTxOut(txid *chainhash.Hash) []*apitypes.TxOut {
	...
	for i := range txouts {
		...
		// types may need to be updated to match pfcd.
		if spk.Type != "invalid" && !apitypes.IsValidScriptClass(spk.Type) {
			log.Warnf(`The ScriptPubKey's type "%s" is not known to pfcdata! ` +
				`Update apitypes or debug pfcd.`)
		}
...
func (db *WiredDB) GetStakeVersions(blockHash string, count int32) (*pfcjson.GetStakeVersionsResult, error) {
...
// on the stake version with which pfcdata is compiled with (chaincfg.Params),
...
func (db *WiredDB) GetSummaryByHash(hash string, withTxTotals bool) *apitypes.BlockDataBasic {
	...
	if withTxTotals {
		...
		var totalFees, totalOut pfcutil.Amount
...
func sumOutsTxRawResult(txs []pfcjson.TxRawResult) (sum float64) {
	for _, tx := range txs {
...
func makeExplorerBlockBasic(data *pfcjson.GetBlockVerboseResult, params *chaincfg.Params) *exptypes.BlockBasic {
...
func makeExplorerTxBasic(data pfcjson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *exptypes.TxBasic {
...
func makeExplorerAddressTx(data *pfcjson.SearchRawTransactionsResult, address string) *dbtypes.AddressTx {
...
func (db *WiredDB) GetExplorerBlock(hash string) *exptypes.BlockInfo {
	...
	for _, tx := range data.RawSTx {
		...
		case stake.TxTypeSSGen:
			...
			// well.
			if stx.Fee > 0 {
				log.Tracef("Vote with fee! %v, %v PFC", stx.Fee, stx.Fees)
			}
	...
	getTotalFee := func(txs []*exptypes.TrimmedTxInfo) (total pfcutil.Amount) {
		for _, tx := range txs {
	...
	getTotalSent := func(txs []*exptypes.TrimmedTxInfo) (total pfcutil.Amount) {
		for _, tx := range txs {
...
func trimmedTxInfoFromMsgTx(txraw pfcjson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *exptypes.TrimmedTxInfo {

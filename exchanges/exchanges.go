...
const (
	...
	Binance     = "binance"
	BinanceURL  = "https://api.binance.com/api/v1/ticker/24hr?symbol=PFCBTC"
	Bittrex     = "bittrex"
...
// PfcExchanges maps tokens to constructors for PFC-BTC exchanges.
var PfcExchanges = map[string]func(*http.Client, *BotChannels) (Exchange, error){
	Binance:  NewBinance,
	Bittrex:  NewBittrex,
...
// opposed to a PicFight-to-Bitcoin Exchange.
func IsBtcIndex(token string) bool {
	_, ok := BtcIndices[token]
	return ok
}

// IsPfcExchange checks whether the given token is a known PicFight-BTC exchange.
func IsPfcExchange(token string) bool {
	_, ok := PfcExchanges[token]
	return ok
}

// Tokens is a new slice of available exchange tokens.
func Tokens() []string {
	tokens := make([]string, 0, len(BtcIndices)+len(PfcExchanges))
	var token string
	for token = range BtcIndices {
		tokens = append(tokens, token)
	}
	for token = range PfcExchanges {
		tokens = append(tokens, token)
	}
	return tokens
}
...
// guaranteed is a price. For PicFight exchanges, the volumes will also be
// populated.

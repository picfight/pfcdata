...
import (
	...
	"github.com/picfight/pfcdata/db/pfcpg/v2"
	"github.com/picfight/pfcdata/db/pfcsqlite/v2"
	"github.com/picfight/pfcdata/exchanges"
	exptypes "github.com/picfight/pfcdata/explorer/types"
	"github.com/picfight/pfcdata/gov/agendas"
	"github.com/picfight/pfcdata/gov/politeia"
	"github.com/picfight/pfcdata/mempool/v2"
	m "github.com/picfight/pfcdata/middleware/v2"
	"github.com/picfight/pfcdata/pubsub"
	pstypes "github.com/picfight/pfcdata/pubsub/types"
...
func _main(ctx context.Context) error {
	...
	// Connect to pfcd RPC server using a websocket.
	ntfnHandlers, collectionQueue := notify.MakeNodeNtfnHandlers()
	pfcdClient, nodeVer, err := connectNodeRPC(cfg, ntfnHandlers)
	if err != nil || pfcdClient == nil {
		return fmt.Errorf("Connection to pfcd failed: %v", err)
	}

	defer func() {
		if pfcdClient != nil {
			log.Infof("Closing connection to pfcd.")
			pfcdClient.Shutdown()
			pfcdClient.WaitForShutdown()
		}
	...
	curnet, err := pfcdClient.GetCurrentNet()
	if err != nil {
		return fmt.Errorf("Unable to get current network from pfcd: %v", err)
	}
	log.Infof("Connected to pfcd (JSON-RPC API v%s) on %v",
	...
	baseDB, cleanupDB, err := pfcsqlite.InitWiredDB(&dbInfo,
		notify.NtfnChans.UpdateStatusDBHeight, pfcdClient, activeChain, cfg.DataDir,
		requestShutdown)
	defer cleanupDB()
	if err != nil {
	...
	if usePG {
		...
		mpChecker := rpcutils.NewMempoolAddressChecker(pfcdClient, activeChain)
		chainDB, err := pfcpg.NewChainDBWithCancel(ctx, &dbi, activeChain,
		...
		if err = auxDB.VersionCheck(pfcdClient); err != nil {
			return err
		}
	...
	Heights := func() (dbHeight, nodeHeight, baseDBHeight, auxDBHeight int64, err error) {
		_, nodeHeight, err = pfcdClient.GetBestBlock()
		if err != nil {
	...
	if baseHeight > -1 {
		orphaned, err := rpcutils.OrphanedTipLength(ctx, pfcdClient, baseHeight, baseDB.DB.RetrieveBlockHash)
		if err != nil {
	...
	if usePG && auxHeight > -1 {
		orphaned, err := rpcutils.OrphanedTipLength(ctx, pfcdClient, auxHeight, auxDB.BlockHash)
		if err != nil {
	...
	if usePG {
		...
		blockHash, nodeHeight, err := pfcdClient.GetBestBlock()
		if err != nil {
			return fmt.Errorf("Unable to get block from node: %v", err)
		}

		block, err := pfcdClient.GetBlockHeader(blockHash)
		if err != nil {
			return fmt.Errorf("unable to fetch the block from the node: %v", err)
		}

		// bestBlockAge is the time since the pfcd best block was mined.
		...
		// expected height of the best block from pfcd now should be this.
	...
	collector := blockdata.NewCollector(pfcdClient, activeChain, baseDB.GetStakeDB())
	if collector == nil {
	...
	// transactions that appear in the picfight blockchain. If corrupted data is
	// is found, its deleted pending the data update that restores valid data.
	agendasInstance, err := agendas.NewAgendasDB(pfcdClient,
	...
	// data stored in github repositories away from the picfight blockchain. It also
	...
	tracker, err := agendas.NewVoteTracker(activeChain, pfcdClient,
	...
	if displaySyncStatusPage {
		...
		latestDBBlockHash, err := pfcdClient.GetBlockHash(dbHeight)
		if err != nil {
	...
	webMux.With(explore.SyncStatusAPIIntercept).Group(func(r chi.Router) {
		// Mount the pfcdata's REST API.
		r.Mount("/api", apiMux.Mux)
		// Setup and mount the Insight API.
		if usePG {
			insightApp = insight.NewInsightApi(pfcdClient, auxDB,
	...
	getSyncd := func(updateAddys, updateVotes, newPGInds bool,
		...
		if cfg.BlockPrefetch {
			pfc := rpcutils.NewBlockPrefetchClient(pfcdClient)
			defer func() {
				pfc.Stop()
	...
	ensureSync := func() error {
		...
		_, height, err := pfcdClient.GetBestBlock()
		if err != nil {
			return fmt.Errorf("unable to get block from node: %v", err)
		}
	...
	// Ensure all side chains known by pfcd are also present in the base DB
	// and import them if they are not already there.
	if cfg.ImportSideChains {
		log.Info("Primary DB -> Now retrieving side chain blocks from pfcd...")
	...
	// Ensure all side chains known by pfcd are also present in the auxiliary DB
	// and import them if they are not already there.
	if usePG && cfg.ImportSideChains {
		// First identify the side chain blocks that are missing from the DB.
		log.Info("Aux DB -> Retrieving side chain blocks from pfcd...")
	...
	// Monitors that fetch the latest updates from pfcd will be launched next.
	...
	// pfcd notifies of block 200000 while pfcdata has only reached 1000 in
	...
	// already registered for notifications with pfcd but the monitors below are
	// not started, notifications will fill up the channels, only to be
	// processed after sync. This is also incorrect since pfcd might notify of a
	...
	// Register for notifications from pfcd. This also sets the daemon RPC
...
func connectNodeRPC(cfg *config, ntfnHandlers *rpcclient.NotificationHandlers) (*rpcclient.Client, semver.Semver, error) {
	return rpcutils.ConnectNodeRPC(cfg.PfcdServ, cfg.PfcdUser, cfg.PfcdPass,
		cfg.PfcdCert, cfg.DisableDaemonTLS, true, ntfnHandlers)
}
...
func listenAndServeProto(ctx context.Context, wg *sync.WaitGroup, listen, proto string, mux http.Handler) {
	...
	go func() {
		var err error
		if proto == "https" {
			err = server.ListenAndServeTLS("pfcdata.cert", "pfcdata.key")
		} else {
			err = server.ListenAndServe()
		}
		// If the server dies for any reason other than ErrServerClosed (from
		// graceful server.Shutdown), log the error and request pfcdata be
		// shutdown.

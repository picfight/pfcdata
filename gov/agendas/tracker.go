...
type VoteDataSource interface {
	GetStakeVersionInfo(int32) (*pfcjson.GetStakeVersionInfoResult, error)
	GetVoteInfo(uint32) (*pfcjson.GetVoteInfoResult, error)
	GetStakeVersions(string, int32) (*pfcjson.GetStakeVersionsResult, error)
}

// pfcd does not supply vote counts for completed votes, so the tracker will
...
type VoteTracker struct {
	...
	blockVersion   int32
	stakeVersion   uint32
	stakeInfo      *pfcjson.GetStakeVersionInfoResult
	voteInfo       *pfcjson.GetVoteInfoResult
	summary        *VoteSummary
	ringIndex      int
	ringHeight     int64
	blockRing      []int32
...
// Since version could technically be updated without turning off pfcdata,
...
func (tracker *VoteTracker) refreshRCI() (*pfcjson.GetVoteInfoResult, error) {
	oldVersion := tracker.Version()
	v := oldVersion
	var err error
	var voteInfo, vinfo *pfcjson.GetVoteInfoResult
...
func rciBlocks(voteInfo *pfcjson.GetVoteInfoResult) int64 {
...
func (tracker *VoteTracker) fetchBlocks(voteInfo *pfcjson.GetVoteInfoResult) ([]int32, uint32, error) {
	blocksToRequest := 1
	...
	var block pfcjson.StakeVersions
...
func (tracker *VoteTracker) refreshSVIs(voteInfo *pfcjson.GetVoteInfoResult) (*pfcjson.GetStakeVersionInfoResult, error) {
...
// Once all resources have been retrieved from pfcd, update VoteTracker fields.
func (tracker *VoteTracker) update(voteInfo *pfcjson.GetVoteInfoResult, blocks []int32,
	stakeInfo *pfcjson.GetStakeVersionInfoResult, stakeVersion uint32) {

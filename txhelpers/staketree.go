...
func BuildStakeTree(blocks map[int64]*pfcutil.Block, netParams *chaincfg.Params,
...
func TicketsInBlock(bl *pfcutil.Block) ([]chainhash.Hash, []*wire.MsgTx) {
...
func TicketTxnsInBlock(bl *pfcutil.Block) ([]chainhash.Hash, []*pfcutil.Tx) {
	tickets := make([]chainhash.Hash, 0)
	ticketTxns := make([]*pfcutil.Tx, 0)
	for _, stx := range bl.STransactions() {
		if stake.DetermineTxType(stx.MsgTx()) == stake.TxTypeSStx {
			h := stx.Hash()
			tickets = append(tickets, *h)
...
func VotesInBlock(bl *pfcutil.Block) []chainhash.Hash {

...
// on the ChainDBRPC receiver.
func (pgb *ChainDBRPC) UpdateChainState(blockChainInfo *pfcjson.GetBlockChainInfoResult) {
...
// DB. Side chains known to pfcd are listed via the getchaintips RPC. Each block
...
func (pgb *ChainDB) AddressHistory(address string, N, offset int64,
	...
	log.Infof("%s: %d spent totalling %f PFC, %d unspent totalling %f PFC",
		address, balance.NumSpent, pfcutil.Amount(balance.TotalSpent).ToCoin(),
		balance.NumUnspent, pfcutil.Amount(balance.TotalUnspent).ToCoin())
...
SPENDING_TX_DUPLICATE_CHECK:
	for _, f := range addressUTXOs.PrevOuts {
		...
		// outpoint because vin:i valuein is not reliable from pfcd.
...
func MakeCsvAddressRows(rows []*dbtypes.AddressRow) [][]string {
	...
	for _, r := range rows {
		...
		csvRows = append(csvRows, []string{
			...
			strValidMainchain,
			strconv.FormatFloat(pfcutil.Amount(r.Value).ToCoin(), 'f', -1, 64),
...
func (pgb *ChainDB) UpdateChainState(blockChainInfo *pfcjson.GetBlockChainInfoResult) {
	if pgb == nil {
		return
	}
	if blockChainInfo == nil {
		log.Errorf("pfcjson.GetBlockChainInfoResult data passed is empty")
		return
	}
...
// GetChainwork fetches the pfcjson.BlockHeaderVerbose and returns only the ChainWork
